...
import { MousePointer2, Ruler, Square, Hash, Scaling } from "lucide-react";
...
export function TakeoffToolbar({
  ...
  return (
    <div className="flex max-w-full items-center gap-0.5 overflow-x-auto rounded-md border border-border bg-card/80 p-0.5">
      ...
      {onSearchClick && (
        <button
          type="button"
          title="Find in plan (Ctrl+F)"
          onClick={onSearchClick}
          className="ml-1 shrink-0 rounded-sm border border-border px-2 py-1 text-xs text-muted-foreground hover:bg-surface-overlay hover:text-foreground"
        >
          Find
        </button>
      )}
